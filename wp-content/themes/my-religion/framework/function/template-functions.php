...
function my_religion_get_page_classes($cmsmasters_option, $classes = false) {
	$browser = new Browser();
	if (
	...
	echo 'cmsmasters_' . $cmsmasters_option['my-religion' . '_theme_layout'] . ' ';

	if ($cmsmasters_option['my-religion' . '_fixed_header']) {
		echo 'fixed_header ';
	}

	if ($cmsmasters_option['my-religion' . '_header_top_line']) {
		echo 'enable_header_top ';
	}

	if ($cmsmasters_option['my-religion' . '_header_styles'] != 'default' && $cmsmasters_option['my-religion' . '_header_styles'] != 'fullwidth') {
		echo 'enable_header_bottom ';
	}

	if ($cmsmasters_option['my-religion' . '_header_styles'] == 'r_nav') {
		echo 'enable_header_right ';
	}

	if ($cmsmasters_option['my-religion' . '_header_styles'] == 'c_nav') {
	...
	if ($cmsmasters_header_overlaps == '') {
		$cmsmasters_header_overlaps = $cmsmasters_option['my-religion' . '_header_overlaps'];
	}
...
function my_religion_header_top($cmsmasters_option) {
	if ($cmsmasters_option['my-religion' . '_header_top_line']) {
		echo '<div class="header_top" data-height="' . esc_attr($cmsmasters_option['my-religion' . '_header_top_height']) . '">' . 
			'<div class="header_top_outer">' . 
				'<div class="header_top_inner">';
					
					if (
						$cmsmasters_option['my-religion' . '_header_top_line_add_cont'] !== 'none'
					) {
						echo '<div class="header_top_right">';
						
						
						if (
							$cmsmasters_option['my-religion' . '_header_top_line_add_cont'] == 'social' && 
							isset($cmsmasters_option['my-religion' . '_social_icons'])
						) {
							my_religion_social_icons();
						} elseif (
							$cmsmasters_option['my-religion' . '_header_top_line_add_cont'] == 'nav' && 
					...
					if ($cmsmasters_option['my-religion' . '_header_top_line_short_info'] !== '') {
						echo '<div class="header_top_left">' . 
							'<div class="meta_wrap">' . 
								stripslashes($cmsmasters_option['my-religion' . '_header_top_line_short_info']) . 
							'</div>' . 
						'</div>';
					} 
...
function my_religion_header_mid($cmsmasters_option) {
	echo '<div class="header_mid" data-height="' . esc_attr($cmsmasters_option['my-religion' . '_header_mid_height']) . '">' . 
		'<div class="header_mid_outer">' . 
			...
			if (
				...
				isset($cmsmasters_option['my-religion' . '_header_donations_but']) && 
				$cmsmasters_option['my-religion' . '_header_donations_but'] && 
				isset($cmsmasters_option['my-religion' . '_header_donations_but_text']) && 
				$cmsmasters_option['my-religion' . '_header_donations_but_text'] != '' && 
				$cmsmasters_option['my-religion' . '_header_styles'] != 'c_nav'
			) {
					...
					'<div class="header_donation_but_wrap_inner">' . 
						'<div class="header_donation_but">' . 
							'<a href="' . esc_url(get_permalink($cmsmasters_donations_form_page)) . '" class="cmsmasters_button">' . 
								'<span>' . esc_html($cmsmasters_option['my-religion' . '_header_donations_but_text']) . '</span>' . 
							'</a>' . 
						'</div>' . 
					'</div>' . 
				'</div>';
			...
			if (
				...
				if (
					$cmsmasters_option['my-religion' . '_header_add_cont'] == 'cust_html' && 
					$cmsmasters_option['my-religion' . '_header_add_cont_cust_html'] !== ''
				) {
				echo '<div class="slogan_wrap">' . 
					'<div class="slogan_wrap_inner">' . 
						'<div class="slogan_wrap_text">' . 
							stripslashes($cmsmasters_option['my-religion' . '_header_add_cont_cust_html']) . 
						'</div>' . 
					'</div>' . 
				'</div>';
				} elseif (
					$cmsmasters_option['my-religion' . '_header_add_cont'] == 'social' && 
					isset($cmsmasters_option['my-religion' . '_social_icons'])
				) {
					my_religion_social_icons();
				}
			...
			if (
				...
				echo '<div class="cmsmasters_dynamic_cart_wrap">';
					my_religion_woocommerce_cart_dropdown(); 
				echo '</div>';
			}
			
			
			if (
				$cmsmasters_option['my-religion' . '_header_search'] && 
				$cmsmasters_option['my-religion' . '_header_styles'] != 'c_nav'
			) {
			...
			if (
				$cmsmasters_option['my-religion' . '_header_styles'] == 'default' || 
				$cmsmasters_option['my-religion' . '_header_styles'] == 'fullwidth'
			) { 
				...
				'<div class="mid_nav_wrap">' . 
					'<nav role="navigation">';
						...
						$nav_args = array( 
							...
							'link_before' => 		'<span class="nav_item_wrap">', 
							'link_after' => 		'</span>', 
							'fallback_cb' => 		'my_religion_fallback_menu' 
						);
...
function my_religion_header_bot($cmsmasters_option) {
	if (
		$cmsmasters_option['my-religion' . '_header_styles'] != 'default' &&
		$cmsmasters_option['my-religion' . '_header_styles'] != 'fullwidth'
	) {
		echo '<div class="header_bot" data-height="' . esc_attr($cmsmasters_option['my-religion' . '_header_bot_height']) . '">' . 
...
/* Get Logo Function */
function my_religion_logo() {
	$cmsmasters_option = my_religion_get_global_options();
	
	
	if ($cmsmasters_option['my-religion' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['my-religion' . '_logo_title'] != '') {
			$blog_title = stripslashes($cmsmasters_option['my-religion' . '_logo_title']);
		} else {
			$blog_title = (get_bloginfo('name')) ? get_bloginfo('name') : 'My Religion';
		}
		
		
		if ($cmsmasters_option['my-religion' . '_logo_subtitle'] != '') {
			$blog_descr = stripslashes($cmsmasters_option['my-religion' . '_logo_subtitle']);
		} else {
			$blog_descr = (get_bloginfo('description')) ? get_bloginfo('description') : esc_html__('Default Logo Subtitle', 'my-religion');
		}
		...
		echo '<a href="' . esc_url(home_url('/')) . '" title="' . esc_attr($blog_title) . '" class="logo">' . "\n\t" . 
			'<span class="logo_text_wrap">' . 
				...
				($cmsmasters_option['my-religion' . '_logo_subtitle'] ? '<span class="title_text">' . esc_html($blog_descr) . '</span>' : '') . 
			'</span>' . 
		'</a>';
	} else {
		...
		if ($cmsmasters_option['my-religion' . '_logo_url'] == '') {
			...
			$logo_img = explode('|', $cmsmasters_option['my-religion' . '_logo_url']);
			...
			if ($cmsmasters_option['my-religion' . '_logo_url_retina'] != '') {
				$logo_img_retina = explode('|', $cmsmasters_option['my-religion' . '_logo_url_retina']);
...
/* Logo Styles */
function my_religion_theme_logo_styles() {
	$cmsmasters_option = my_religion_get_global_options();
	...
	if ($cmsmasters_option['my-religion' . '_logo_type'] == 'text') {
		if ($cmsmasters_option['my-religion' . '_logo_custom_color']) {
			$out .= "
...
";
		...
		$header_mid_height = (($cmsmasters_option['my-religion' . '_header_mid_height'] !== '') ? $cmsmasters_option['my-religion' . '_header_mid_height'] : '108');
...
function my_religion_footer_logo($cmsmasters_option) {
	if (
		$cmsmasters_option['my-religion' . '_footer_type'] == 'default' && 
		$cmsmasters_option['my-religion' . '_footer_logo']
	) {
		list($logo_footer_width, $logo_footer_height) = getimagesize(get_template_directory() . '/img/logo_footer.png');
		
		
		if ($cmsmasters_option['my-religion' . '_footer_logo_url'] == '') {
			...
			$footer_logo_img = explode('|', $cmsmasters_option['my-religion' . '_footer_logo_url']);
			...
			if ($cmsmasters_option['my-religion' . '_footer_logo_url_retina'] != '') {
				$footer_logo_img_retina = explode('|', $cmsmasters_option['my-religion' . '_footer_logo_url_retina']);
...
function my_religion_get_footer_custom_html($cmsmasters_option) {
	if (
		(
			$cmsmasters_option['my-religion' . '_footer_type'] == 'default' && 
			$cmsmasters_option['my-religion' . '_footer_html'] !== ''
		) || (
			$cmsmasters_option['my-religion' . '_footer_type'] == 'small' && 
			$cmsmasters_option['my-religion' . '_footer_additional_content'] == 'text' && 
			$cmsmasters_option['my-religion' . '_footer_html'] !== ''
		)
	) {
		echo '<div class="footer_custom_html_wrap">' . 
			'<div class="footer_custom_html">' . 
				do_shortcode(stripslashes($cmsmasters_option['my-religion' . '_footer_html'])) . 
			'</div>' . 
		'</div>';
	}
}
...
function my_religion_get_footer_nav($cmsmasters_option) {
	if (
		has_nav_menu('footer') && 
		(
			(
				$cmsmasters_option['my-religion' . '_footer_type'] == 'default' && 
				$cmsmasters_option['my-religion' . '_footer_nav']
			) || (
				$cmsmasters_option['my-religion' . '_footer_type'] == 'small' && 
				$cmsmasters_option['my-religion' . '_footer_additional_content'] == 'nav'
			)
		)
	) {
...
function my_religion_get_footer_social_icons($cmsmasters_option) {
	if (
		isset($cmsmasters_option['my-religion' . '_social_icons']) && 
		(
			(
				$cmsmasters_option['my-religion' . '_footer_type'] == 'default' && 
				$cmsmasters_option['my-religion' . '_footer_social']
			) || (
				$cmsmasters_option['my-religion' . '_footer_type'] == 'small' && 
				$cmsmasters_option['my-religion' . '_footer_additional_content'] == 'social'
			)
		)
	) {
		my_religion_social_icons();
...
function my_religion_page_heading() {
	$cmsmasters_option = my_religion_get_global_options();
	...
	if (
		...
		$cmsmasters_header_overlaps = $cmsmasters_option['my-religion' . '_header_overlaps'] ? 'true' : 'false';
		
		$cmsmasters_heading_alignment = $cmsmasters_option['my-religion' . '_heading_alignment'];
		$cmsmasters_heading_scheme = $cmsmasters_option['my-religion' . '_heading_scheme'];
		
		$cmsmasters_breadcrumbs = $cmsmasters_option['my-religion' . '_breadcrumbs'] ? 'true' : 'false';
	}
			...
			if ($cmsmasters_heading == 'custom') {
				...
				if (is_search()) {
					...
					if (!empty($wp_query->found_posts)) {
						echo sprintf(esc_html(_n('1 search result for: %2$s', '%1$d search results for: %2$s', $wp_query->found_posts, 'my-religion')), $wp_query->found_posts, get_search_query());
					} else {
						echo sprintf(esc_html__('0 search results for: %s', 'my-religion'), get_search_query());
					}
				} elseif (is_archive()) {
					if (is_author()) {
						if (get_the_author_meta('first_name') != '' || get_the_author_meta('last_name') != '') {
							echo sprintf(esc_html__('Author: %1$s (%2$s %3$s)', 'my-religion'), '<span class="vcard">' . get_the_author() . '</span>', get_the_author_meta('first_name'), get_the_author_meta('last_name'));
						} else {
							echo sprintf(esc_html__('Author: %s', 'my-religion'), '<span class="vcard">' . get_the_author() . '</span>');
						}
					} else {
...
/* Get Page Heading Function */
function my_religion_theme_page_heading_styles() {
	$cmsmasters_option = my_religion_get_global_options();
	...
	if (
		...
		$cmsmasters_heading_height = $cmsmasters_option['my-religion' . '_heading_height'];
		$cmsmasters_heading_bg_color = $cmsmasters_option['my-religion' . '_heading_bg_color'];
		$cmsmasters_heading_bg_img_enable = $cmsmasters_option['my-religion' . '_heading_bg_image_enable'] ? 'true' : 'false';
		$cmsmasters_heading_bg_img = $cmsmasters_option['my-religion' . '_heading_bg_image'];
		$cmsmasters_heading_bg_rep = $cmsmasters_option['my-religion' . '_heading_bg_repeat'];
		$cmsmasters_heading_bg_att = $cmsmasters_option['my-religion' . '_heading_bg_attachment'];
		$cmsmasters_heading_bg_size = $cmsmasters_option['my-religion' . '_heading_bg_size'];
	}
...
function my_religion_social_icons() {
	$cmsmasters_option = my_religion_get_global_options();
...
<div class=\"social_wrap\">
	...
	foreach ($cmsmasters_option['my-religion' . '_social_icons'] as $cmsmasters_social_icons) {
...
function my_religion_theme_social_icons_styles() {
	$cmsmasters_option = my_religion_get_global_options();
...
function my_religion_thumb($cmsmasters_id, $type = 'post-thumbnail', $link = true, $group = false, $preload = true, $highImg = false, $fullwidth = true, $show = true, $attachment = false, $unique = false, $link_icon = false, $placeholder_icon = 'cmsmasters_theme_icon_image') {
	$args = array( 
...
function my_religion_thumb_small($cmsmasters_id, $type = 'post', $w = 100, $h = 100, $show = true) {
...
/* Get Heading Function */
function my_religion_get_comments($class = false, $show = false) { 
	$out = '';
	
	
	if (comments_open()) {
		$out .= '<span class="cmsmasters_comments' . ($class ? ' ' . $class : '') . '">' . 
			'<a class="cmsmasters_theme_icon_comment" href="' . esc_url(get_comments_link()) . '" title="' . esc_attr__('Comment on', 'my-religion') . ' ' . esc_attr(get_the_title()) . '">' . 
...
function my_religion_prev_next_posts($cmsmasters_post_type_name = '') {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post"><span class="post_nav_sub">' . esc_html__('Previous', 'my-religion') . '<span class="post_nav_type"> ' . $cmsmasters_post_type_name . ' </span>' . esc_html__('Link', 'my-religion') . '</span>%link<span class="cmsmasters_prev_arrow"><span></span></span></span>');
		
		next_post_link('<span class="cmsmasters_next_post"><span class="post_nav_sub">' . esc_html__('Next', 'my-religion') . '<span class="post_nav_type"> ' . $cmsmasters_post_type_name . ' </span>' . esc_html__('Link', 'my-religion') . '</span>%link<span class="cmsmasters_next_arrow"><span></span></span></span>');
...
function my_religion_sharing_box($title_box = false, $tag = 'h3') {
	...
	echo "<aside class=\"share_posts\">
		...
		<div class=\"share_posts_inner\">
			<a href=\"https://www.facebook.com/sharer/sharer.php?display=popup&u={$page_link}\">" . esc_html__('Facebook', 'my-religion') . "</a>
			<a href=\"https://twitter.com/intent/tweet?text=" . urlencode(html_entity_decode(sprintf(esc_attr__("Check out '%s' on %s website", 'my-religion'), $social_title, $website_name), ENT_QUOTES, 'UTF-8')) . "&url={$page_link}\">" . esc_html__('Twitter', 'my-religion') . "</a>
			<a href=\"" . esc_url("https://pinterest.com/pin/create/button/?url={$page_link}&media={$pinterest_img}&description={$social_title}") . "\">" . esc_html__('Pinterest', 'my-religion') . "</a>
		</div>
	</aside>
";
}
...
function my_religion_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'my-religion') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function my_religion_related($tag = 'h3', $more_posts = '', $no_posts = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									my_religion_thumb(get_the_ID(), 'cmsmasters-blog-masonry-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
...
function my_religion_author_avatar($template_type = 'page') {
...
/* Get Category List */
function my_religion_get_the_category_list($cmsmasters_id, $taxonomy, $sep = '', $before = '', $after = '') {
...
if (!function_exists('my_religion_get_post_pings')) {

function my_religion_get_post_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (pings_open($id) && sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'my-religion') . '</' . esc_html($tag) . '>' . "\n" .
